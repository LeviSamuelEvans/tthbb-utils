...
def save_plot(canvas, sample_type, var_code, node_label):
    directory = f"plots_inclusive_norm/{var_code}/{node_label}"
...
def draw_atlas_label(canvas, label_text="Internal"):
    ...
    latex.DrawLatex(
        0.1, 0.91, f"#font[72]{{ATLAS}} #scale[0.6]{{#font[42]{{{label_text}}}}}"
    )
    return latex
...
def plot_histograms(var, var_range, node, node_label, chains, selections, weights):
    ...
    for sample in ["data", "ttbb", "tt", "ttH"]:
        ...
        h = ROOT.TH2F(
            hist_name, "", 50, var_range[0], var_range[1], 100, 0, 20
        )  # 20 for 2b, 10 for rest, 50 for ttH
        ...
        chains[sample].Draw(
            f"{node}:{var}>>{hist_name}", f"{weight} * {selection}", draw_option
        )
...
def plot_norm_histograms(var, var_range, node, node_label, chains, selections, weights):
    ...
    for sample in ["data", "ttbb", "tt", "ttH"]:
        ...
        for ix in range(1, h.GetNbinsX() + 1):
            col_total = sum(
                h.GetBinContent(ix, iy) for iy in range(1, h.GetNbinsY() + 1)
            )
            if col_total > 0:
...
if __name__ == "__main__":
    ...
    weight_dict = {"data": "1", "ttbb": mc_weight, "tt": mc_weight, "ttH": mc_weight}
    ...
    for var, var_range in var_dict.items():
        for node_label, node in node_dict.items():
            plot_norm_histograms(
                var, var_range, node, node_label, chain_dict, sel_dict, weight_dict
            )
