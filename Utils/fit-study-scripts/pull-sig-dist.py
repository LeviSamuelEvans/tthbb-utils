...
pull_significances = np.abs(best_fit_values - 0) / np.sqrt(1 - np.maximum(up_uncertainties, down_uncertainties)**2)

# Filter positive pull significances and remove non-finite values (like inf or NaN)
positive_pull_significances = pull_significances[(pull_significances > 0) & np.isfinite(pull_significances)]
...
observed_values, bin_edges, _ = ax.hist(positive_pull_significances, bins=10, color='lightblue', edgecolor='black', alpha=0.7, density=True, label="Data")
...
shapiro_text = ('Shapiro-Wilk Test: W Statistic=%.3f, p=%.3f, α = %.3f' % (stat, p, alpha))
ax.text(0.99, 0.82, shapiro_text, transform=ax.transAxes, fontsize=10, verticalalignment='top', horizontalalignment='right', bbox=dict(boxstyle='round', facecolor='white', alpha=0.5))
...
ax.text(0.95, 0.65, stats_text, horizontalalignment='right',
        verticalalignment='top', transform=ax.transAxes, bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))
