    ...
    with open(file_path, "r") as f:
        for line in f:
            if "parameter:" in line:
                ...
                norm_data[param]["up"] = float(
                    parts[1].split(":")[1].split("(")[0].strip()
                )
                norm_data[param]["down"] = float(
                    parts[2].split(":")[1].split("(")[0].strip()
                )
    ...
    for param, data in norm_data.items():
        ...
        if "up" in data and "down" in data:
            ...
            if key_prefix + "_up" in replacements:
                updated_values[key_prefix + "_up"] = data["nominal"] / (
                    (data["up"] / replacements[key_prefix + "_up"])
                )
            elif (
                key_prefix in replacements
            ):  # Fallback if _up is not in replacements (not needed for down)
                updated_values[key_prefix] = data["nominal"] / (
                    (data["up"] / replacements[key_prefix])
                )
            else:
                print(
                    f"Could not find {key_prefix}_up or {key_prefix} in replacements"
                )  # Debug print

            if key_prefix + "_down" in replacements:
                updated_values[key_prefix + "_down"] = data["nominal"] / (
                    (data["down"] / replacements[key_prefix + "_down"])
                )
            elif (
                key_prefix in replacements
            ):  # Fallback if _up is not in replacements (not needed for down)
                updated_values[key_prefix] = data["nominal"] / (
                    (data["up"] / replacements[key_prefix])
                )  # simple overwite the up from above (hacky)
            else:
                print(
                    f"Could not find {key_prefix}_down or {key_prefix} in replacements"
                )  # Debug print
