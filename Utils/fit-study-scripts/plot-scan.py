...
def read_yaml_files(folder_path, systematic):
...
def plot_likelihood_scan(data, systematic):
    ...
    plt.grid(True)
    plt.savefig(f'LHscan_{systematic}.png')
    plt.savefig(f'LHscan_{systematic}.pdf', dpi=700)
    plt.close()
    print(f"INFO: Plots saved as LHscan_{systematic}.png and LHscan_{systematic}.pdf")

def main(folder_path, systematics):
    for systematic in systematics:
        data, systematic = read_yaml_files(folder_path, systematic)
        if data: 
            plot_likelihood_scan(data, systematic)
        else:
            print(f"WARNING: No data found for {systematic}, skipping plot.")

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Plot 1D likelihood scans for given systematics.")
    parser.add_argument('-p', '--folder-path', type=str, help="Path to the folder containing scan YAML files.")
    parser.add_argument('-s','--systematics', type=str, help="Comma-separated list of systematics to plot.")
