...
def download_samples(sample_list, rse, login_info=None, nominal_only=False, dsid_map=None):
    """Download samples from Rucio.

    Args:
        sample_list (str): Path to a text file with a list of samples to download.
        rse (str): Replica SErvice to download from.
        login_info (dict, optional): Rucio account name and authentication host. Defaults to None.
        nominal_only (bool, optional): If True, only download nominal samples. Defaults to False.
        dsid_map (dict, optional): A dictionary mapping DSIDs to folder names. Defaults to None.

    Raises:
        Exception: If no folder is found for a given DSID.
    ...
    with open(sample_list, 'r') as f:
        for sample in f:
            ...
            if not folder:
                raise Exception(f"No folder found for DSID {dsid}. Skipping...")
