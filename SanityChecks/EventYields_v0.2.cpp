//                                                          //
// Basic script to check event yields in L2 samples LE v0.3 //
//                                                          //

// - Requires a .txt file containting all root files to run over and print outputs to terminal/ log files 
// - compile using g++ -o EventYields_v0.2 EventYields_v0.2.cpp `root-config --cflags --libs`
...
void getEntries(const std::string& sampleName, const char* filename) {
...
void processDirectory(const std::string& dir_path) {
    ...
    while ((entry = readdir(dir))) {
        ...
        if (S_ISDIR(buf.st_mode)) {
            ...
            getEntries(sampleName, full_path.c_str());
        }
    }
    closedir(dir);
}
...
int main() {
    ...
    std::string logFilePath = "EventYields_1l.log";
